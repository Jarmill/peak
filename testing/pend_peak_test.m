...
X_swing = (energy_gap^2 >= epsilon^2);
% X_swing_pos = energy_gap >= epsilon;
% X_swing_neg = energy_gap <= -epsilon;
X_wait  = [energy_gap^2 <= epsilon^2; quad_lqr >= delta];
X_lqr   = [energy_gap^2 <= epsilon^2; quad_lqr <= delta];
...
% X_swing_2 = [X_swing; c <= 0];
% 
% f = {f_lqr, f_swing, f_swing, f_swing};
% X = {X_lqr, X_wait, X_swing_1, X_swing_2};
